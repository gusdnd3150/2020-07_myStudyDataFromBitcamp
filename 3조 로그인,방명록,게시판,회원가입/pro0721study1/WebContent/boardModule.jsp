...
<jsp:useBean id="paging" class="morning.Paging" scope="request"></jsp:useBean>
<jsp:useBean id="refresh" class="java.lang.String" scope="request"></jsp:useBean>
<head>
<meta charset="UTF-8">
...
<body>
	<table>
		...
		<c:choose>
			...
			<c:when test="${!empty articleList }">
				<c:forEach items="${articleList}" begin="${paging.minBound}" end="${paging.maxBound}" var="page">
		...
		<tr>
			<td colspan="5" style="border-bottom: 0px;"><c:forEach var="i" begin="1" end="${paging.maxNum}">
					<c:if test="${nowPage==i}">
						<p style="font-weight: bold; display: inline;">${i}</p>
					</c:if>
					<c:if test="${nowPage!=i}">
						<a href="search.article?nowPage=${i}">${i}</a>
					</c:if>
				</c:forEach></td>
		</tr>
		<tr>
